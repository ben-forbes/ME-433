...
# NOTES FROM CLASS
# default filter -  moving average filter - special type of FIR Filter
# we have the old array and now we have new data that pushed the old data out - then average then 

# RC Filter
# can double up on these and then we get a greater slope
# Knee frequency = cutoff frequency

# IIR Filter
# s = read()
# estimate of value = A*old estimate + B*s  scaled by A and B where A+B = 1
# If A is large, I think s is noisy, is B is large, then I am trusting that s is not noisy
# old est = estimate

# FIR Filter
# individually multiply each index in the array by a different coefficient - to choose the bandwidth
