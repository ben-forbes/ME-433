...
ipv4 =  ipaddress.IPv4Address("192.168.1.66")
...
def webpage():
    ...
    <h1>Stopwatch:</h1><br>
    <p class="dotted">Press the button to get the amount of time that has passed since the last button press.</p><br>
    <form accept-charset="utf-8" method="POST">
    <button class="button" name="STOPWATCH" value="STOP" type="submit">STOPWATCH</button></a></p></form>
    <span style="color: deeppink;">Time elapsed:{watch}</span></p><br>
    </body></html>
    """
    return html
...
def buttonpress(request: HTTPRequest):
    ...
    if "OFF" in raw_text:
        ...
        led.value = False
    # if the stopwatch button was pressed
    if "STOP" in raw_text:
        # reset the time that the stopwatch is based off of
        global start_time
        start_time = time.monotonic()
    #  reload site
    with HTTPResponse(request, content_type=MIMEType.TYPE_HTML) as response:
...
while True:
    try:
        ...
        if (clock + 30) < time.monotonic():
            ...
            clock = time.monotonic()
            temp_test = str(c_to_f(microcontroller.cpu.temperature))
        watch = time.monotonic() - start_time
        print(watch)
